...
### Motivation is not a speed knob

The first separate fits freed all seven parameters and are best read as a negative control; their full parameter sets and histories remain in the repository. The selected `h0` and `h−` points had three-seed norms of 14.1 and 15.3, but evaluating the `h−` point on the `h0` data gave 9.8. Because that point was feasible in the `h0` search, **the selected `h0` point was demonstrably not an optimum, despite that Dakota run satisfying its stopping test.** The `h−` search reached its iteration limit and put desired speed near its lower bound, while neighbor and wall parameters also moved by factors of two to four. These fits show optimizer and parameter compensation, not how motivation changes people.

The cleaner test freezes the five interaction parameters at set C and profiles only desired speed `v0` and time gap `T`. For `h0`, Dakota found a one-seed norm of 6.79 over 33 residuals, or 1.18 assumed standard deviations per residual; the three-seed check gave 7.43, or 1.29 per residual. The surface is a shallow diagonal valley rather than an isolated point: grid points (`v0`, `T`) = (0.97, 0.65), (1.13, 0.83) and (1.30, 0.83) have norms 7.9, 8.6 and 7.6. At Dakota's selected point the three seed-specific norms span 7.42 to 7.74, so small differences within the valley are not practically distinguishable. The pair is constrained jointly, but neither coordinate is identified precisely. Both two-parameter Dakota runs reached their 20-iteration cap without satisfying the distance or expected-improvement convergence tests, although their histories had settled into the same regions as the grid.

For `h−`, the result is qualitatively different. The surface has no interior minimum in the tested range: it falls towards the upper `T` boundary and is broad along that edge. At `T = 1.2` s, the grid norms are 13.4, 12.3, 12.5 and 14.7 as `v0` runs from 1.30 to 1.80 m/s. Dakota's reported pair (1.485, 1.200) is therefore false precision; the defensible result is `T` at the tested bound and `v0` weakly determined. Widening the box is not the answer. The earlier `T`-only fit allowed 1.9 s and still gave 11.7, essentially the same as the two-parameter result, 11.8 with one seed and 11.9 with three. **The `h0` condition can be represented by a `v0`–`T` valley, but no pair in the tested domain represents `h−`. Motivation is not a speed knob in this model.**

The one-seed grids also provide the relevant transfer test without another simulation. Neither condition's profile point transfers to the other:

| profile point | target h0 | target h− |
|---|---:|---:|
| h0 valley point (1.30, 0.83) | 7.6 | 15.8 |
| h− edge point (1.47, 1.20) | 11.3 | 12.3 |

The failed `h−` fit is diagnostic. At Dakota's edge point, the three-seed mean flow is high in eight of nine runs. In two of the three 1.2 m runs density is 40 to 45 % high while speed is 23 to 29 % low; at 3.4 m one run is 46 % too dense and 41 % too slow. Increasing `T` slows discharge by piling people up, whereas the experiment combines similar gate flow with a less dense, faster upstream state. This pattern points towards condition-dependent spacing or interaction behaviour, but a residual pattern alone does not identify its mechanism. Run 110 is also retained transparently: it has 63 tracked participants, does not empty in any of the three seeds at the selected `h0` profile point, and contributes its finite partial flow, density and speed to three of the 33 residuals.

We tested the simplest spacing interpretation directly by fixing `v0` and `T` at the `h0` valley and profiling radius and neighbor range over the same seven-by-seven grid. The one-seed surface has an interior minimum at radius 0.167 m and neighbor range 0.147 m, but its norm is 14.6, only slightly below the transferred `h0` value of 15.8 and worse than the `v0`–`T` edge result. The three-seed check is worse still, norm 17.6, with two stalled seeds in one run. **Static spacing does not absorb motivation in this two-parameter slice either.** The result still motivates a dynamical state that can alter local spacing and interaction behaviour, but it does not establish that radius and neighbor range are that state.

{{< figure
    src="motivation_profile.png"
    caption="One-seed residual profiles with all unshown parameters fixed. Left and centre: desired speed v0 against time gap T for h0 and h−; right: radius against neighbor range for h− with v0 and T fixed at the h0 valley. Open circles mark grid minima and crosses the fixed set-C coordinates. The common colour scale is clipped at norm 30. The h0 surface is a shallow valley, the h− v0–T surface runs to a broad T boundary, and the spacing profile does not improve on it. Three-seed checks give norms 7.4 for the h0 valley, 11.9 for the h− edge and 17.6 for the spacing minimum."
>}}
...
| CrowdQueue set C | | 9.7 | 58 / 0 / 5 |
| CrowdQueue set D | | 11.3 | 52 / 10 / 1 |
| joint | 7.9 | 12.0 | 61 / 0 / 2 |
...
**The joint set is close to the CrowdQueue specialist on CrowdQueue and three times worse than either Hermes set on Hermes.** The single search we ran found no parameter set that meets the stated tolerances in both experiments; it found one that nearly matches the narrow-gate specialist at the price of the wide-bottleneck fit. Whether a better compromise exists elsewhere in the parameter space, this search cannot say.
...
- **Some behaviour is not a parameter.** The `v0`–`T` profile fits the baseline condition as a valley but cannot fit low motivation anywhere in the tested range; extending `T` from 1.2 to 1.9 s buys nothing. Profiling radius and neighbor range at the baseline valley also fails its three-seed check. A model that treats motivation as a speed knob, or assumes that one static spacing pair represents it, will be wrong in exactly the situations that matter for safety.
...
The first lesson is that a validation study must audit its own data path before it audits the model. Recheck the coordinate system, geometry, units, population count, initial conditions, arrival process, exit definition, stopping rule, time step and trajectory-writer lifecycle. Compare the analysis code on a deliberately simple case with a hand calculation, and test that a run which fails, stops early or drops an agent cannot be mistaken for a physically meaningful zero. The drivers now retain an `evaluation_status.json` audit record for each simulation, including completion status, crossed population and exception type; failed or too-short runs use a zero response as Dakota's finite fallback, while incomplete runs retain their finite partial observables and are marked separately. This makes failed evaluations visible without making the optimizer parser consume non-finite values. **The finite fallback is a bookkeeping safeguard, not a physical observation and not a good surrogate target.**
