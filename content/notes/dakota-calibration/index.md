...
The separate fits are a deliberately cautionary negative control, not direct evidence of a motivation mechanism. The `h0` search stopped after 38 evaluations with its stopping criteria met; the `h−` search used 96 evaluations and reached its iteration limit without meeting them. The `h−` desired speed, 0.819 m/s, is close to its lower bound of 0.80 m/s. Comparing the resulting points is therefore not a comparison of two equally converged optima. Even after recomputing both norms from three independent seeds, the fits are poor (14.1 and 15.3 over 33 and 27 residuals). **The parameter difference cannot distinguish static compensation from the non-identifiability already seen between sets C and D.** Desired speed falls by 0.41 m/s and the time gap rises by 11 %, but the neighbor range changes by a factor of four, the wall range by a factor of three and a half, the wall strength falls by more than half, and the radius and neighbor strength also move. These are compensating parameter combinations, not estimates of how motivation changes human walking.

The mechanism is clearer in the observables than in the parameter table. With the `h0` point, simulated flow is nearly flat across corridor widths, about 1.31 to 1.36 persons per second, while the experiment spans about 1.00 to 1.38; simulated speed is 30 to 45 % too low in most runs. The `h−` point lowers `v0`, which pulls the mean flow towards the lower-motivation level, but it overestimates density in the 1.2 m corridors (about 2.9 and 4.3 persons per square metre against 1.8 and 2.6). **Here `v0` behaves mainly as a flow scaler, while neither fit reproduces the width- and condition-dependent structure.**

We then performed the cheap swap test that the separate fits require. Each selected point was evaluated on the other condition with three seeds per run, without refitting. The entries are norms computed from the three-seed means, in the same residual units as the table above.

| parameter set used | target h0 | target h− |
|---|---:|---:|
| h0 separate fit | 14.1 | 18.8 |
| h− separate fit | 9.8 | 15.3 |

The swap matrix is asymmetric because the conditions contain different runs and the model is nonlinear, but it does not show that either condition owns a distinct parameter set: the `h−` point transfers to `h0` at least as well as its own fit, and the `h0` point is only moderately worse on `h−`. **This is exactly why a lower residual alone is not a diagnosis.**

Finally, we froze the five static interaction parameters at set C and varied only `v0` and `T` on a seven-by-seven grid. The grid used one fixed seed per point; its minima were checked with three independent seeds. The approximate grid minima were (`v0`, `T`) = (1.30, 0.83) for `h0` and (1.47, 1.20) for `h−`, with confirmed norms 7.2 and 12.2. Dakota's separate 20-iteration two-parameter runs selected (1.246, 0.788) and (1.485, 1.200); three-seed checks at those exact points gave norms 7.4 and 11.9. The low-motivation minimum lies at the upper time-gap bound, so it is a boundary indication rather than a well-estimated value. **This nested profile is a cleaner bridge to a dynamical motivation model than the seven-parameter comparison: it asks the stated question while preventing the other five parameters from absorbing the condition.**

{{< figure
    src="motivation_profile.png"
    caption="Normalized residual surface when only desired speed v0 and time gap T vary and the five static parameters remain at set C. Each panel is a seven-by-seven one-seed grid; the open circle is the grid minimum and the cross is set C. The h− minimum is at the upper T bound. Three-seed checks at the two grid minima give norms 7.2 and 12.2 for h0 and h−; checks at Dakota's selected points give 7.4 and 11.9."
>}}

There is a further Dakota-specific warning in these runs. Failed evaluations were kept finite by returning zero observables, which creates clipped residuals—for example, the repeated flow fallback near -16.67 in normalized units. That is useful for making the run auditable, but it creates cliffs that can mislead a Gaussian-process surrogate and helps explain slow convergence. **For a production calibration, failure capture should be separated from physical zero, with Dakota's declared recovery mechanism or a documented smooth penalty; the profile grid above avoids fitting a surrogate through those cliffs.**

The profile is not yet a dynamical model or a held-out validation. **A further bridge to a dynamical model would keep one common set of static interaction parameters, allow only condition-specific changes in `T` and `v0` (or in a motivation state that controls them), and compare that model with the fully separate fits on held-out runs.** The present profile is the cheap parameter-identification check needed before making that stronger claim.
