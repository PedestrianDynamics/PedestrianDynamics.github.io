...
where \(\theta\) is the parameter vector, \(y_i(\theta)\) the simulated observable, \(\hat{y}_i\) the measured one, and \(w_i\) a weight set from the assumed uncertainty, 6 % of the measured flow and 10 % of the measured density and speed. Each evaluation of \(f\) costs three JuPedSim runs, and it is noisy because the simulation is stochastic, so the optimizer never differentiates it. Instead it fits a Gaussian process to all evaluations made so far, which gives a prediction \(\mu(\theta)\) and a standard deviation \(s(\theta)\) at every untried point, and it picks the next point where the expected improvement over the best value \(f_{\min}\) seen so far is largest:
...
- **Two parameter sets, not one.** Two optimizer starts give sets that fit equally well and differ by factors of two to four. The data do not identify the parameters; they constrain combinations of them.
...
- **A pipeline audit.** Between the first draft and this one, reviews found participants missing from simulations, unwritten final trajectory frames and a geometry error. Each was corrected and every dependent result recomputed; the Hermes steady-phase observables changed by less than half a percent. The audit record is in the repository.
