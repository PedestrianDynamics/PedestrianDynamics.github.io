...
- **A fitted parameter shift is not yet a mechanism.** Extending the `v0`–`T` profile changes the low-motivation optimum substantially and reproduces much of the front occupancy contrast, but the same fit remains systematically too slow across wider corridors. The spacing slice instead develops a stall cliff. These are useful model diagnostics; neither proves what psychological motivation is.
