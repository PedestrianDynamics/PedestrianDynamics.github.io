...
Finally, we froze the five static interaction parameters at set C and varied only `v0` and `T` on a seven-by-seven grid. The grid used one fixed seed per point; its minima were checked with three independent seeds. The approximate minima were (`v0`, `T`) = (1.30, 0.83) for `h0` and (1.47, 1.20) for `h−`, with confirmed norms 7.2 and 12.2. The low-motivation minimum lies at the upper time-gap bound, so it is a boundary indication rather than a well-estimated value. **This nested profile is a cleaner bridge to a dynamical motivation model than the seven-parameter comparison: it asks the stated question while preventing the other five parameters from absorbing the condition.**

{{< figure
    src="motivation_profile.png"
    caption="Normalized residual surface when only desired speed v0 and time gap T vary and the five static parameters remain at set C. Each panel is a seven-by-seven one-seed grid; the open circle is the grid minimum and the cross is set C. The h− minimum is at the upper T bound. Three-seed checks at the two grid minima give norms 7.2 and 12.2 for h0 and h−."
>}}
